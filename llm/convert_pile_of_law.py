...
class ShardedLaw(IterableDataset):

    def __init__(self, split):
        self.dataset = hf_datasets.load_dataset(path='pile-of-law/pile-of-law',
                                                name='all',
                                                split=split,
                                                streaming=True)
...
def build_hf_law_dataset(split: str) -> IterableDataset:
...
def main(args: Namespace) -> None:
    ...
    """
    columns = {'text': 'str', 'downloaded_timestamp': 'str', 'url': 'str'}

    for (split, split_new_name, expected_num_samples) in [
        ('train', 'train', 7500000),
        ('validation', 'val', 2500000),
    ]:
        ...
        # Get samples
        dataset = build_hf_law_dataset(split=split)
